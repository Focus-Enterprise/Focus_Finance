  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-background border-t">
        <nav className="flex items-center justify-between h-16 px-6" role="navigation" aria-label="Navegação mobile">
          {navigation.map((item) => (
            <NavLink
              key={item.name}
              to={item.href}
              ...
              className={({ isActive }) =>
                cn(
                  'flex flex-col items-center justify-center space-y-1 py-2 px-1 rounded-lg transition-colors min-w-0 flex-1 mx-1',
                  isActive 
